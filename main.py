...
if not DISCORD_TOKEN or not DISCORD_BOT_TOKEN:
...
class DiscordClient:
        ...
        if self.session_id:
            ...
            payload = {
                ...
                "d": {
                    ...
                    "presence": {"status": "online", "since": 0, "activities": [{
                        "name": "Custom Status",
                        "type": 4,
                        "state": None,
                        "timestamps": {
    ...
    async def update_presence(self, is_playing, track_data=None):
        presence_payload = {
            "op": 3,
            "d": {
                "status": "dnd" if is_playing else "online",
                "since": 0,
                "activities": [track_data, {
                    "name": "Custom Status",
                    "type": 4,
                    "state": None,
                    "timestamps": {
        ...
        try:
            async for message in self.websocket:
                ...
                if "wss://event" in payload.get("uri", ""):
                    is_playing = payload["payloads"][0]["events"][0]["event"]["state"]["is_playing"]
                    track_data = {
                        "type": 2,
                        "name": "Spotify",
                        "assets": {
                            "large_image": f"spotify:{payload['payloads'][0]['events'][0]['event']['state']['item']['album']['images'][0]['url'].split('/')[-1]}",
                            "large_text":
                                payload["payloads"][0]["events"][0]["event"]["state"]["item"]["name"]
                        },
                        "details": payload["payloads"][0]["events"][0]["event"]["state"]["item"]["name"],
                        "state": payload["payloads"][0]["events"][0]["event"]["state"]["item"]["artists"][0][
                            "name"],
                        "timestamps": {
                            "start": payload["payloads"][0]["events"][0]["event"]["state"]["timestamp"] -
                                     payload["payloads"][0]["events"][0]["event"]["state"]["progress_ms"],
                            "end": payload["payloads"][0]["events"][0]["event"]["state"]["timestamp"] +
                                   payload["payloads"][0]["events"][0]["event"]["state"]["item"]["duration_ms"],
                        },
                        "party": {
                            "id": "spotify:98141664303927296"
                        },
                        "sync_id": payload["payloads"][0]["events"][0]["event"]["state"]["item"]["id"],
                        "flags": 48,
                        "metadata": {
                            "context_uri": payload["payloads"][0]["events"][0]["event"]["state"]["context"][
                                "uri"],
                            "album_id": payload["payloads"][0]["events"][0]["event"]["state"]["item"]["album"][
                                "id"],
                            "artist_ids": [x["id"] for x in
                                           payload["payloads"][0]["events"][0]["event"]["state"]["item"][
                                               "album"]["artists"]],
                            "type": "track",
                            "button_urls": []
                        }
    ...
    async def connect(self):
        while True:
            ...
            try:
                ...
                print(f"{Fore.YELLOW}Retrying connection in 5 seconds...{Style.RESET_ALL}")
                self.token = None
                await asyncio.sleep(5)
